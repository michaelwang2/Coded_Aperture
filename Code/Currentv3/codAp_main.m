...
% import
path = 'RAW/';
I0 = imread(strcat(path, 'codAp_2_0_gray.bmp'));
I1 = imread(strcat(path, 'codAp_2_1_gray.bmp'));
I2 = imread(strcat(path, 'codAp_2_2_gray.bmp'));
I3 = imread(strcat(path, 'codAp_2_3_gray.bmp'));
I4 = imread(strcat(path, 'codAp_2_4_gray.bmp'));
I5 = imread(strcat(path, 'codAp_2_5_gray.bmp'));
I6 = imread(strcat(path, 'codAp_2_6_gray.bmp'));
I7 = imread(strcat(path, 'codAp_2_7_gray.bmp'));
I8 = imread(strcat(path, 'codAp_2_8_gray.bmp'));
I9 = imread(strcat(path, 'codAp_2_9_gray.bmp'));
I10 = imread(strcat(path, 'codAp_3_0_gray.bmp'));

% crop
codAp_2_0_gray_undist = imcrop(im2double(I0(:, :)), [78, 91, 3277, 2211]);
codAp_2_1_gray_undist = imcrop(im2double(I1(:, :)), [274, 143, 3123, 2102]);
codAp_2_2_gray_undist = imcrop(im2double(I2(:, :)), [286, 181, 2983, 2001]);
codAp_2_3_gray_undist = imcrop(im2double(I3(:, :)), [274, 226, 2849, 1906]);
codAp_2_4_gray_undist = imcrop(im2double(I4(:, :)), [322, 258, 2733, 1831]);
codAp_2_5_gray_undist = imcrop(im2double(I5(:, :)), [373, 272, 2628, 1758]);
codAp_2_6_gray_undist = imcrop(im2double(I6(:, :)), [409, 291, 2526, 1691]);
codAp_2_7_gray_undist = imcrop(im2double(I7(:, :)), [487, 299, 2436, 1634]);
codAp_2_8_gray_undist = imcrop(im2double(I8(:, :)), [479, 317, 2350, 1575]);
codAp_2_9_gray_undist = imcrop(im2double(I9(:, :)), [532, 328, 2262, 1520]);
codAp_3_0_gray_undist = imcrop(im2double(I10(:, :)), [625, 341, 2186, 1474]);
...
for i = 1:numBlurred
    ...
    deconvImages{i} = deconvL2(im2double(rgb2gray(I_cropped)), mKers{i}, we, max_it); % Use the right orientation Kernel!!!!!!
    times(i) = toc;
    fprintf('Image %d\n', i);
end
